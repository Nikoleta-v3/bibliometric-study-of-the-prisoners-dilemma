...
\node (title) at (-.5, 1.2) {DATA SCRAPING};
...
\draw (term) edge[out=0, in=90, ->, thick] node [] {} (arcas);
\draw (arcas) edge[out=-90, in=0, ->, thick] node [] {} (db);
...
\draw (if_api) edge[out=-90, in=180, ->, thick, dotted] node [above] {stop
collection} (db);
...
-6.5) {\textbf{LIST} lexicographical similar titles \textbf{REMOVE} duplicates};
...
-8.5) {\textbf{LIST} lexicographical similar authors \textbf{Fix} authors name};
...
-10.5) {\textbf{LIST} lexicographical similar authors \textbf{Fix} authors
name};
