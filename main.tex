...
Following the original tournaments it was proven that direct reciprocity can
...
new strategies. In evolutionary settings strategies can be learn to adapt their
strategy over time based upon what has been effective and what has not. In
essence reinforcement learning techniques can be used to train strategies for the
iterated prisoner's dilemma. A strategy must represented in a generic strategy
...
% A way of overcoming this limitation is to use an undirected
% evolution process as a reinforcement learning technique to discover new effective
% strategies. Reinforcement learning is based on the idea that the tendency to
% produce an action which corresponds to a favourable results should be strengthened,
% whereas it should be weakened if it corresponds to unfavourable results.
% Reinforcement learning algorithms can be evolutionary, for example
% genetic algorithms~\cite{Axelrod1987}, but is not necessary for example
% Q-learning~\cite{Sandholm1996}.

% , such an example is the genetic algorithm used by Axelrod
% in~\cite{Axelrod1987}. In order to use such process
% Axelrod required a representation of an iterated prisoner's dilemma strategy such
% that the algorithm could optimise; he required a parametrised strategy able to
% change it's behaviour and adapt. These will re refereed to as structured strategies.
% In this section we will cover papers that explore new structures for IPD strategies
% or new training algorithms discover effective strategies.

In~\cite{Axelrod1987} Axelrod, having realised the limitations of his evolutionary
work, sicked to use a reinforcement algorithm called the genetic algorithm to train
strategies. Axelrod decided to considered deterministic strategies that took into account
...
possible combinations. Axelord therefore used a list of 64 C's and D's to represent
different strategies. This archetype is reffeed to as a lookup table. A set of deterministic responses based on
the opponents \(m\) last moves; for example \cite{Axelrod1987} considered \(m=3\).
He then used a genetic algorithm in order to fine tune the responses and
obtain a trained strategy.
a genetic algorithm was used to optimise a structured
strategy refereed to as a look up table. A genetic algorithm is a search heuristic
...
In 1989~\cite{nowak1989}, Nowak and Sigmund proposed a structure for studying a
set of very simple strategies. These are strategies that remember only the previous
...
In 1992~\cite{Nowak1992} the same authors used a population 99 reactive to study
which one would take over. The winning strategy was called \textbf{Generous Tit for Tat}
given by the triplet \((1, 0, \frac{2}{3})\), which was their answer as to how to
deal with noise environment. Moreover in
1990~\cite{Nowak1990} they expanded their structure and gave a formal definition
of a memory one strategy.
...
of exploring strategies and we discuss some more recent work that has been done
using this structure.

by Harrald and Fogel (1996). The~\cite{Harrald1996}
network used memory with length of 3 and the actions were encoded as continuous values between -1
and 1 where 1 meant complete cooperation. 6 input nodes represented 3 previous steps of the player
and the opponent. There was a hidden layer of N fully connected nodes and an output node that
produced values from the range <-1;1>. 
