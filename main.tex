...
conducted by the authors. It focuses on the analysis of the 
...
network theory. %TODO: alter at the end
...
metadata of articles that satisfied the request message. The raw metadata are 
...
the prisoner's dilemma have been discussed in the following section we explore
the people that have written about their game and their connections.
...
unique author, within the data set, is represented as a node of the network and
...
within the data set. In order to address the problem the Levenshtein Distance was
used. The Levenshtein Distance highlighted the entries with a small difference
...
\subsubsection{Co-authors network from the prisoner's dilemma data set}
...
The \authors vertices represent each of the unique authors in the data set. 
...
Vertices' weight corresponds to the number of publications an author has within
the data set. The ten authors with the highest number of publications are given
...
node weight has been applied is now given in Figure~\ref{fig:authors_network_weighted}.
...
connectivity of the vertices does seem to exists, with a single large cluster
...
More insights can be gained by reviewing several network measures. The first
...
More specifically, the most central author for this work in Matjaz Perc.
...
prisoner's dilemma we perform a comparison this section. How well connected the
network of authors for the prisoner's dilemma is compared to two other networks.
These are network constructed from two new data set collected for the purpose
of this work. The data collected are on two different topics within game theory,
these are:

\begin{itemize}
    \item price of anarchy. key: price of anarchy;
    \item auction games. key: auction game theory.
\end{itemize}

The open source project Arcas was used again, this time using the following 
keyword respectively for each topic,
...
the topic. In comparison, a total of 2103 articles are examined here for auction 
games. Auction games are a well studied topic for many year, the earliest entry
was published in 1974. Finally, 3860 different names are examined. The frequency
of the prisoner's dilemma, for both articles and authors, lies between these two
data sets. In Figure~\ref{fig:timeplots_anarchy_auction} a timeplot for each topic
are viewed, both topics have an increasing trend over the years. Though price of
anarchy is clearly a new topic compared to auction games.
...
The immediate step is to compare the networks we construct from all three 
data set. The aim of the comparison is to gain insight as to how collaborative
...
networks close to 3 and the density of all networks is very small, less than
0.005.
...
consider the degree distributions. The normalised described distributions of all
three networks are given in Figure~\ref{fig:degrees_dist}. They have been normalise
such that the frequencies are sum to one. The distributions appear to be similar
...
is used. Kruskal-Wallis allow us to compare the medians of two or more distributions.
The test returns a p-value of 0.29. Thus, there is no significant difference in
the degree distributions of the three networks.
...
increasing trend there have been two jumps. A decreased jump in 1960 and 
and an increasing on around 1970. Finally, the price of anarchy had a big
increase in authorship over the years 2004-2006.
...
are normally distributed. Thus, a Krustal-Wallis test will be performed to test
...
both the weighted betweenness and weighted page rank we reject the null hypothesis.
Thus there seems to exist a significant difference of the medians for these two
measures. A post-hoc test could be carried to further explore which medians 
are significantly different. Though for the aims of this analysis we have shown
that even though the degrees in the topics do follow similar distributions
in some fields more central author exist.
...
theory. In the section to follow we return in the exploration of the co-authorships
within the field. The collaboration of some well known authors alongside authors
that appear to be central in our network are explored.
...
cliques of the network that has been constructed. Cliques are subsets of vertices,
all adjacent to each other. 

A total of 779 cliques exists within the network and the max node number of a
clique is 21. The clique with the 21 authors is that of the co-authorship from
the paper~\cite{Knight2016}. Though a big number of cliques exists in this 
section we will only examine those of authors that have been discussed several
times within this work.

The first clique is that of R. Axelrod. The sub graph, shown in Figure
\ref{fig:axelrods_clique}, consists of 4 vertices and 3 edges. The central
node is Axelrod himself and the edge weights are all equal to 1. Thus 
for this subgraph we examined the betweenness, closeness and page rank centrality
(all without weights). The results are shown in Table~\ref{table:axelrods_clique}.

Furthermore, the density of the sub graph is equal to 0.5 and the average degree
in 1.5. All authors are connected to Axelrod with a step of one. That is because
all these authors have written with Axelrod. These are the papers~\cite{Axelrod1988,
Axelrod1984, Wu1995}.
...
Ashlock is larger than the previous one we reviewed. It has a total of 24 vertices
and 38 edges. The density is equal to 0.137 and the average degree is 3.16.
Moreover, based on all three centrality measures, examined for the previous subgraph,
Ashlock appears to be the most central node of this subgraph. Table~\ref{table:ashlock_clique}
present the 10 most central authors.
...
of the subgraph was calculated using three different measures and are shown in
Table~\ref{table:perc_clique}. The density is equal to 0.02088 and the average
degree is 5.533. 
...
The short path that connected these authors is also retrieved and is the following:
(M. Nowak - A. Traulsen- Y. Moreno - M. Perc. Thus the authors that connect them
are two of the most central authors. The answer as to why these people are connected
can be seen if we consider the topic the have writen for. The similarity on these
people work is evolunationary process and spatial tournaments. M. Perc is a 
signicant author within the topic with some of his most well received and most 
citated works being~\cite{Perc2010, Perc2013, Perc2013b, Perc2008}.
...
\subsubsection{Conclusions from the analysis.}
\newpage
