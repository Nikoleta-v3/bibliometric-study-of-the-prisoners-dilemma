...
\end{abstract}
%TODO The last sentence should be replaced with a sentence such as:
%The network grows over time. So does the main cluster of the network.
%The most central people are the people in the main cluster
%and they all write in the evolutonary on networks topic
...
LDA to identify dominant topics in the PD fields and analyses
the networks corresponding to these topics individually.

Though several of the approaches used in this manuscript have previously been
carried out, as already discussed, the novelty here is combining the approaches
as well as applying them to a new data set. The methodology used in this
manuscript, which includes the data collection and a preliminary analysis of the
data set, are covered in Section~\ref{section:methodology}. The results are
presented in Section~\ref{section:results} and summarised in
Section~\ref{section:conclusion}. The results of the manuscript fit two
categories. Initially, the results on the research topics of the PD and secondly
the results of the co-authorship network.
%TODO add a result sentence?


\section{Methodology}\label{section:methodology}

%TODO reference software here
...
\begin{center}
    \includestandalone{assets/tex/scrape}
\end{center}
    

\begin{center}
\includestandalone{assets/tex/data_collection_diagram}
\end{center}
...
we looked at a number of paper that we know are relevant and identified that
these are the relevant keywords. Other terms such as donation game, etc could
also have been used but were not. Whilst the results obtained, do generalise to
the overall stated goal of the paper they are inferred only from the dataset
used which is available at.


Four prominent journals in the field and a preprint server
...
existed within the title, the abstract or the text are included in the analysis.
These search terms were chosen after identifying that these are terms that
appear in papers known to be relevant in the field. Note that there are several
other terms that could have been used as well, such as donation game. The results of
the manuscript do generalise to the overall stated goals
...
%TODO VK to have a thing about this sentence

The latest data collection was performed on the \(30^{\text{th}}\) November
2018. Following the automatic collection the data set was cleaned manually. More
specifically, papers for which the search terms existed within the text were
check to reassure their relevance to the PD topic. A number of articles were
removed in this step. Moreover, the data were checked for duplicates. In case
of duplicates the preprint version of an article (collected fromarXiv) was dropped.
A total of \manual articles were not
collected but were manually added because they are of interest to the field.
Examples of such papers include~\cite{Flood1958} the first publication on the
...
The process of obtaining the final and clean data set used in the manuscript is
shown in Figure~\ref{figure:colection_process}.
This data set has been archived and is available at~\cite{pd_data_2018}.

\begin{figure}[!hbtp]
    \begin{center}
        \includestandalone[width=\textwidth]{assets/tex/data_collection_diagram}
    \end{center}
    \caption{The generating process of~\cite{pd_data_2018}.}\label{figure:colection_process}
\end{figure}

%TODO add that sentence to summarise again our choices in the data collection:
%reviewer's comment
...
Section~\ref{section:results} using the co-authorship network. The collaborative
behaviour of authors will also be explored in co-authorship networks which
correspond to their publications research topics. These topics and their
reverence over time are also presented in the next section.
...
coherence score measures the degree of semantic similarity between high scoring
...
The LDA models and the topic coherence for each model were calculating using the
...
exclusivity of the high ranking words of the topics is decreasing as the number
of topics increases. A number of topics $n=5$ has a better exclusivity value
than the model of $n=6$, and it's coherence score is 0.406 (which is closed to
...
The five topics in the PD publications identified by the data set of this
...
Table~\ref{table:topics_per_year} for each time period. For
