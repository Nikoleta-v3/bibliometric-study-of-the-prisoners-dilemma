...
data set and for each journal individually. This is denoted as,
...
fields, it is still a constant number. The data we are handling here are a time
series. Moreover it appears that our time series has a trend. This is shown
...
The results show that out data is non stationary: the \(p\) value is greater than
0.005.

For projecting the behaviour of the field of the new years we are using an ARIMA
model which is able to handel non stationary data. Though the details for the
ARIMA~\cite{brockwell2013} models are not going to be given here, the parameters
