...
continued interest, and though it is a collaborative field, it is not
necessarily more collaborative than other science fields. The co-authorship
network suggests that authors are focused on their communities and no many
...
applied to the LDA topics, and the co-authorship network is analysed a whole
...
\begin{enumerate}
    ...
    \item Is on topic worth to start investigating?
    \item How do the research topics change over the years?
    ...
    network? and who does this corresponds to the topic networks?
\end{enumerate}
...
probability of a document belonging to a topic is refereed to as the percentage
contribution denoted as \(c\). For example the words and their associated
weights for two topics A and B be:
...
There are a total of \authors authors in~\cite{pd_data_2018} and several of these
authors had had multiple publications collected from the data collection process.
...
PD, a LDA algorithm implemented in~\cite{rehurek_lrec} is applied to the
abstract's of the data set. As mentioned before, the number of topics, which
...
A dimensionality reduction approach called t-Distributed Stochastic Neighbor Embedding
(t-SNE)~\cite{Maaten2008} is applied to the LDA model outputs to visualise the
result. An LDA model outputs an \(N \times n\) matrix - \(N\) rows for \(N\)
...
\(i \in \{1, 2, \dots, n\}\) for \(j \in \{1, 2, \dots, N\}\).
...
of topic 1 are divided into two clusters. The one cluster is closer to documents
from topic 4 and the other to documents from topic 3. In comparison,
\ref{fig:lda_visualisation_five} gives the visualisation of LDA \(n=5\) where
the separation of the documents is more clear. Excluding model LDA \(n=6\)
several models, Figure~\ref{fig:coherence_value_over_number_of_topcis}, have
higher coherence value than the LDA model with \(n=5\). However, the separation
...
dynamics on networks and modeling problems as a PD}. These 5 topics nicely
...
they still attract the interest of academics. Thus, \textbf{any of the topics is
...
models for \(n\) equal to 5, 6 and the optimal number of topics over time. For
...
is 5, 6 or the optimal number of topics of a given period. The performances are
...
PD that has not connections to any other author. The results indicate that the
PD is a collaborative field.

However, both \(G\) and \(\bar{G}\) have a high modularity and a large number of
communities. A high modularity implies that authors create their own publishing
...
spread the knowledge of the field across academic teams.

Thus, \textbf{the PD is indeed a collaborative field but perhaps it is not
more collaborative than other fields}, as there is not effort from the authors
...
an author is 10\% more unlike to collaborate with a collaborator's co-author.
...
\textbf{Topic D also appears to be collaborative}. Topic D is the topic
associated with evolutionary dynamics on networks, and in fact its network is a
sub graph of \(\bar{G}\). It will be demonstrated in the following section that
...
and who does this corresponds to the topic networks?}
...
the results on centrality heavily rely of the main cluster. Matjaz Perc is an
...
It is obvious that in \(G\) the centralities valurs are very low which suggests
...
only the authors connected to the main cluster, to a main group of authors}.
...
of Topic D. From the list of names it is obvious that these authors are
...
attention and publications. In Section~\ref{section:topics} a LDA algorithm was
...
and agent based simulations, evolutionary dynamics on networks and modeling
...
authors tend to collaborate with authors from their communities, but not many
collaborations happen between authors from different communities. This however
