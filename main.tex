...
The immediate step is to evaluate these collaborations and understand why
authors tend to collaborate. This is achieved by studying the cliques of the
network. Cliques are subsets of vertices, all adjacent to each other, also
called complete subgraphs. Cliques can be looked at is a group of individuals
who interact with one another and have worked on the same projects within the
research of the prisoner's dilemma.
...
to look at the type of collaborations that exist within the field. The following
sub graphs have been generated by looking at cliques with a minimum of 5 vertices
that are not just co-authors for a single paper. 
...
two authors. A very common collaborations structure. More interesting cases 
arise, shown in Figure~\ref{fig:cliques_1}. These can be viewed as cases
that people worked on the same topic and almost everyone in the clique has 
collaborated with each other, generating almost a complete graph.
...
It is shown that clusters of authors have been brought together by few 
connecting vertices. The connecting vertices are authors such as Long Wang,
Martjaz Perc, Yamir Moreno and Alex Doud.
...
However noticeable authors such as Nowak, Ashlock, Plotkin and Axelord did not
appear on these networks. The analysis suggests that Plotkin and Axelrod are
authors that do not have many collaborations. On the other hand, Nowak's clique
can be visualised by looking at cliques with minimum 4 vertices,
Figure~\ref{fig:fig:nowak_clique}. Nowak appears to be the connecting author
of 3 different clusters (the minimum size studied is 4 vertices).
Similarly Ashlock has several collaborations as well. Ashlock is a connecting
author of approximately 4 clusters. Those clusters have a minimum size of 3
vertices. Investigating the reasons authors collaborate and this network structures
appear will be explored later on using natural language techniques.

\begin{figure}[!hbtp]
    \includegraphics[width=\textwidth]{./assets/images/Nowak.pdf}
    \caption{Martin Nowak clique network.}\label{fig:nowak_clique}
\end{figure}

\begin{figure}[!hbtp]
    \includegraphics[width=\textwidth]{./assets/images/Ashlock.pdf}
    \caption{Daniel Ashlock clique network.}\label{fig:ashlock_clique}
\end{figure}
