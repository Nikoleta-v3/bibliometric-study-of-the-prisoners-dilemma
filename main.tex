...
\(T\), whereas the cooperator receives \(S\). For a dilemma to exist the payoffs are
subject to constrains (\ref{eq:constrain_one}) and (\ref{eq:constrain_two}).
...
Due to rational behaviour and constrains (\ref{eq:constrain_one}), (\ref{eq:constrain_two})
it would never benefit a player to cooperate. It can shown mathematically that
defecting is the dominant strategy for the one shot prisoner's dilemma. However,
when the game is studied in a manner where prior outcomes matter,
...
works have shown that cooperation can emerge when players interact for more than
one time. One of the most important of these works has been R. Axelrod's as
described in his book~\cite{Axelrod1984} ``The Evolution of Cooperation''.
...
\subsection{Origin and Primal research (1961-1972)}\label{section:origin}
...
% These research mainly focused on the amount of cooperation attained depending on a variety of factors.
...
were publicized and the second tournament received much more attention, with 62 entries
...
none of the variants managed to outperform the pure version, the winner was once
again Tit for Tat. The conclusions made from the tournaments were that the strong
...
In Axelrod's ecological tournament the Tit for Tat continued to thrive and the simulation of
the ecological tournament had shown that the Tit for Tat rule was used by everyone.
Ecological tournament does not offer any evolutionary perspective. It's only an
estimation of the strategies' frequencies based on the tournament payoff matrix.
...
In 1981, Axelrod also study the prisoner's dilemma in an evolutionary context based
on the evolutionary approaches introduced by an attendant of the second tournament
and a well known evolutionary biologist, John Maynard Smith~\cite{Smith1973,
Smith1974, Smith1979}. John Maynard Smith alongside his collaborator George Price
are considered fundamental figures of evolutionary game theory.
In~\cite{Smith1973} they introduced the definition of an Evolutionary Stable 
strategy (ESS).
...
Thus for a strategy to be ESS it must be able to resists any invasion.

The work described in~\cite{axelrod1981}, studied the evolutionary stability of
Tit for Tat and introduce the definition of a collective stability. Although the
strategy was likely to take over the population, its stability was conditional
on the importance of the future of the game, which is represented by a discounting factor
\(w\). Axelrod showed that if \(w\) was sufficiently large, Tit for Tat could resist
invasion by any other strategy. Moreover, he showed how a small cluster of such
players could invade a hostile environment.

Axelrod decided to work on the biological applications of his results in collaboration
with the biologist William Donald Hamilton. According to Richard Dawkins
he was the one to introduce Axelrod to Hamilton's work. Their collaboration~\cite{Axelrod1984}
won the Newcomb-Cleveland prize of the American Association for the Advancement
of Science.
