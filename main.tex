...
\begin{abstract}
    ...
    Prisoner's Dilemma. This is achieved in two parts.

    Firstly, we provide a detailed literature review by reviewing selected
    pieces of work. The timeline has been partitioned into five different
    sections, each reviewing a different aspect of it's research. Secondly, we
    analyse a comprehensive data set which consists with literature of the
    ...
    (3) how collaborative is the field and (4) do people influence the file more
    compared to other fields?
\end{abstract}

\section{Introduction}\label{section:introduction}

Though cooperative behavior seems at odds with the Darwinian principle of
survival of the fittest, cooperation is abundant in nature~\cite{Stewart15348}.
The golden paradigm of understanding these behaviours/concepts, which is to use
...
Axelrod reports on a series of computer tournaments he organised. Participants
...
to design computer strategies to compete. The pioneering work of Axelrod showed
that greedy strategies did very poorly in the long run whereas altruistic
strategies did better.
``The Evolution of Cooperation'' is considered a milestone in the field but it
is not the only one. On the contrary, the PD has attracted attention ever since
the game's origins.

In Section~\ref{section:timeline}, a qualitative description of selected pieces
...
The aim of this paper is to provide a concrete summary of the existing
literature of the PD research. This is done to provide the
research community a review which will allow it's members to understand overall
trends in the field, and already existing results.
...
In this section literature regarding the prisoner's dilemma is reviewed. The
...
Nash~\cite{Nash1951}), in an attempt to delivery the game with a story during a
...
participants simulated rounds of the game. Human subject research has
disadvantages. Humans can behave randomly and in several experiments both the
...
Mack1971}, the physical distance between the players~\cite{Sensenig1972}, the
effect of the opening moves~\cite{Tedeschi1968} and the experimenter, by varying
...
the values used in the rest of the work described here.
...
At least 9 of the original participants competed again and two strategies that
remained the same were Tit For Tat and \textbf{Grudger}. Grudger is a strategy
that will cooperate as long as the opponent does not defect, originally
...
mechanism win-stay, lose-shift. It starts off with a cooperation and then
repeats it's previous move only if it was awarder with a payoff of \(R\) or
\(T\). Pavlov is heavily studied in the literature and similarly to Tit for Tat
it's used in tournaments perform until today and has had many variants trying to
build upon it's success, for example \textbf{PavlovD} and \textbf{Adaptive
Pavlov}~\cite{Li2007}.
...
in real life situations~\cite{Milinski1987}. Moreover, research believed that
the strategy's simplicity was not necessarily a feature, but a limitation~\cite{Beaufils1997}.

Regarding stochastic uncertainties, \cite{Molander1985} proved that in an
environment where \textbf{noise} is introduced two strategies playing Tit for
Tat receive the same average payoff as two Random players (a probability that a
player's move will be flipped) and in a computer tournament with a 10 percent of
noise and Tit for Tat finished sixth out of 21 strategies~\cite{Donninger1986}.
Jonathan Bendor in~\cite{Bendor1991} performed tournament similar to Axelrod's
with noise and a probability of 0.0067 of ending in the next turn. His results
demonstrated the poor performance of Tit for Tat once again and showed that the
highest ranked strategies were more generous ones. His top ranked strategy was
called \textbf{Nice and Forgiving}. Hammerstein~\cite{Hammerstein1984}, pointed
out another weakness of Tit for Tat in noisy environments. If by mistake, one of
two Tit for Tat players makes a wrong move, this locks the two opponents into a
hopeless sequence of alternating defections and cooperations. To overcome this
error~\cite{Wolfgang2006} introduced another more generous variant of Tit for
Tat, \textbf{OmegaTFT}. A second type of stochastic uncertainty is
misperception, where a player's action is made correctly but it's recorder
incorrectly by the opponent. In 1986,~\cite{Wu1995} a strategy
...
Tit for Tat was not so fast to retaliate a defection. All these aforementioned
works hinted that there are at least two ways of coping with unindented
defections, that are due to happen in uncertain environments, generosity and
contrition. Generosity is letting a percentage of defections go unpunished and
contrition, it's lowering a strategy's readiness to defect and a defection.

Regarding the limitations of simplicity, the advantages of complexity were shown
by~\cite{Beaufils1997} in 1997. A new strategy was constructed to have the the
same qualities as those of Tit for Tat except one, Gradual has a memory of the
game since the beginning of it. Gradual punishes defections with a growing
number of defections but after punishing enters a calming state in which it
cooperates for two rounds. In a tournament of 12 strategies, Gradual scored
first and outperformed both Tit for Tat and Pavlov. Another antagonist of
intelligent design with a infnite memory of the game is \textbf{Adaptive Tit for
Tat}. The authors of~\cite{tzafestas-2000a} conducted the exact same tournament
as~\cite{Beaufils1997} with now 13 strategies and their strategy ranked first.
These results are the first of many that illustrated the advantages of
\textbf{memory} in the IPD games. This topic will be discussed in more detail in
...
A.Rogers2007Ctpw}. A team of strategies include strategies that have been been
programmed with a recognition mechanism by default. Once the strategies
recognise one another, one would act as leader and the other as a follower. The
follower then plays as a Cooperator, cooperates unconditionally and the leader
would play as a Defector gaining the highest achievable score. Followers would
behave as Defector towards other strategies to defect their score and help the
leader. In~\cite{A.Rogers2007Ctpw}, a team for the University of Southampton
used teams and recognition patterns and managed to win the 2004 Anniversary
...
Other works, also looked their limitations. Finally, the design of teams and the
notion of recognition patterns were discussed. The next section focuses on a
different aspect of research which is evolutionary dynamics.

\subsection{Evolutionary Dynamics}\label{subsection:evolutionary_dynamics}

So far the emergence of cooperation has been discussed in the contexts of the
the one shot PD game and in IPD tournaments. In the one shoot game cooperation it
can be proven that cooperation fails, the
only Nash equilibrium is that of mutual defection, moreover, in a series of
influencing works Axelrod's argued that reciprocal cooperation will emerge when
...
The importance of understanding the the conditions under which natural selection
can favour cooperative behaviour lies within understanding social behaviour
almost humans and other mammals~\cite{Boyd1987}.

This section covers a number of works in understanding the evolutionary outcomes
of the IPD. Most of these studies have been made possible due to Evolutionary
Game Theory, and several of the approaches used by these works have been
established by biologist John M. Smith~\cite{Smith1973, Smith1974, Smith1979}. A
fundamental figure within evolutionarily game theory and a participant of
...
each round becomes stronger as weaker performers are reduced and eliminated. His
results have been that a handful of nice strategies came to dominate the
population.

Following this ecological tournament Axelrod alread studied the PD
in evolutionary context baseoud. Alongside the biologist William Donald Hamilton
they wrote about the biological applications of the evolutionary dynamics of the
iterated prisoner's dilemma~\cite{Axelrod1984} and won the Newcomb-Cleveland
...
% In Axelrod's odel pairl of individuals were peared gorm a politao and player the
% IPD. The wa are wpro that a par more ine more than t times, w_t 0 < w< 1. Axelrod's
% results showed taht dor a sufficient high w Tit for Tat, nice, were likely
% to become commen. Though Axelrod acreuf collective stable.
...
Boyd \& Lorberbaum\cite{Boyd1987}  have shown that no pure
strategy, including TFT, is evolutionarily stable in the sense that each can be
invaded by the joint effect of two invading strategies when long-term
interaction occurs in the repeated game and future moves are discounted.
This was also independently
proven by~\cite{Pudaite1987}. More over, 
Farrell
\& Ware (1989, Theor. Popul. Biol. 36, 161) have since extended these results to
include finite mixes of pure strategies as well.

Summary, and structured populations.
